...
class TestParser:
    def test_fake_bedfile(self, test_data_dir: Path, bed_parser):
        ...
        with pytest.raises(FileNotFoundError):
            bed_parser.read_methylation_bed(nonexistent_file, {})
    ...
    def test_empty_bedfile(self, test_data_dir: Path, bed_parser):
        ...
        result2 = bed_parser.read_methylation_bed(empty_file, result1)
    ...
    def test_bedmethyl_bedfile(self, test_data_dir: Path, bed_parser):
        ...
        results = bed_parser.read_methylation_bed(
            sample_bedmethyl_bed,
            regions_dict,
        )
        assert list(results.keys())[0] == "chrX_MATERNAL"
        assert len(list(results.keys())) == 1
        region_values = results["chrX_MATERNAL"]
    ...
    def test_chrom_dict_len(self, test_data_dir: Path, data_handler_factory):
        ...
        assert len(handler.methylation_dict) == 1
        assert len(handler.region_dict) == 1
    ...
    def test_remote_dataset_processing(
        ...
        methylation_dict = bed_parser.read_methylation_bed(
        ...
        assert all(key.split(":", 1)[0] == expected_chrom for key in methylation_dict)
