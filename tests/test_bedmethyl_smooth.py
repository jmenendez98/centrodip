...
def test_empty_input_returns_empty_bedgraph():
    out = bedMethyl_LOWESS(
        chrom="chr1",
        bedMethyl=BedTable([], inferred_kind="bed", inferred_ncols=11),
        window_bp=1000,
        cov_conf=1.0,
    )
...
def test_cov_conf_must_be_positive(bt_bedmethyl_linear_start: BedTable):
    with pytest.raises(ValueError, match="cov_conf must be greater than 0"):
        bedMethyl_LOWESS(chrom="chr1", bedMethyl=bt_bedmethyl_linear_start, window_bp=1000, cov_conf=0.0)

    with pytest.raises(ValueError, match="cov_conf must be greater than 0"):
        bedMethyl_LOWESS(chrom="chr1", bedMethyl=bt_bedmethyl_linear_start, window_bp=1000, cov_conf=-5.0)
...
def test_negative_coverage_raises():
    ...
    with pytest.raises(ValueError, match="coverage must be non-negative"):
        bedMethyl_LOWESS(chrom="chr1", bedMethyl=bt, window_bp=1000, cov_conf=1.0)


def test_output_preserves_order_and_has_two_extras(bt_bedmethyl_linear_start: BedTable):
    out = bedMethyl_LOWESS(
        chrom="chr1",
        bedMethyl=bt_bedmethyl_linear_start,
